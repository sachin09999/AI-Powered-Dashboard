...
export function AiInsights() {
    ...
    const handleInitialAnalysis = async () => {
        ...
        try {
            const summaryResult = await summarizeDataTrends({
                data: mockCsvData,
                question: 'Provide a general summary of the dataset.'
            });

            const recommendationsResult = await recommendChartTypes({
            ...
            const aiResponse = (
                <div>
                    ...
                    <p className="mb-2">{recommendationsResult.reasoning}</p>
                    <div className="flex flex-wrap gap-2">
                        {recommendationsResult.recommendedChartTypes.map(chart => (
            ...
            setMessages([{ id: 'error', type: 'ai', content: 'Sorry, I encountered an error. Please try again.' }]);
            console.error(error);
        } finally {
            setIsLoading(false);
        }
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>Ask questions about your data and get instant answers.</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col h-[500px] justify-between">
                {messages.length === 0 && !isLoading ? (
                    <div className="flex flex-1 flex-col items-center justify-center text-center gap-4">
                        ...
                        <Button onClick={handleInitialAnalysis}>Get Initial Data Analysis</Button>
                    </div>
                ) : (
